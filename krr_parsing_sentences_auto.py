...
program = ["agent fire"]
query = ["possibly p and q after program"] #False 0
...
query = ["possibly pieniadze after"] #True 1

fluents_tab_test.append(fluents)
sequences_tab_test.append(sequences)
program_tab_test.append(program)
query_tab_test.append(query)
correct_reults_test.append(True)

program = ["zlodziej wlam", "domownik zablokuj", "zlodziej wlam"]
query = ["program realizable always"] #False 2
...
query = ["active zlodziej in program"] #False 3
...
program = ["domownik zamknij", "zlodziej wlam"]
query = ["active zlodziej in program"] #Ture 4
...
query = ["necessary open after program"] #False 5
...
program = ["bill insertCard"]
query = ["possibly open after program"] #False 6
...
program = ["bill insertCard"]
query = ["realizable always"] #False 7
...
query = ["program realizable always"] # True 8
...
program = ["bill load", "bill shoot"]
query = ["necessary !alive after program"] #True 9
...
program = ["bill shoot"]
query = ["necessary walking after program"] #False 10
...
program = ["bill shoot"]
query = ["realizable always"] # True (daje false)TODO wynikanie  11
...
query = ["possibly !loaded after program"] #True 12
...
query = ["possibly alive after program"] #True 13
...
program = ["bill insertCard"]
query = ["necessary open after"] # True 14
...
program = ["bill insertCard"]
query = ["realizable always"] ## True 15
...
program = ["bill insertCard"]
query = ["possibly open after "] # False 16
...
query = ["active smok in program"] # False 17
...
query = ["possibly smok_zabity after program"] # False 18
...
query = ["active ogrodnik in program"] # False 19
...
program = ["ogrodnik napelnianie", "ogrodnik podlewanie"]
query = ["realizable always"] # True TODO !dawid to check  if true response is ok  20
...
query = ["necessary !tom_has_money and car_running after program"] # True 21
...
query = ["possibly tom_has_money after program"] # True  TODO check (poprawione) 22
...
program = ["Tom turn_on_car", "Thief steal_money"]
query = ["necessary tom_has_money after program"] # False 23
...
query = ["realizable always"] # False (daje true) TODO check (chyba ok bo nie ma zadnego impossible w sequences) 24
...
for i in range(len(fluents_tab_test)):
    inx = i
    fluents = fluents_tab_test[inx]
    sequences = sequences_tab_test[inx]
    program = program_tab_test[inx]
    query = query_tab_test[inx]
...
print(f"correct_reults_test {correct_reults_test}")
